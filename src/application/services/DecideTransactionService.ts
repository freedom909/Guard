...
// application/services/DecideTransactionService.ts
...
export class DecideTransactionService {
  ...
  execute(command: ExecuteTransactionCommand) {
    // 1️⃣ 调用 Domain 决策逻辑
    const result = decideTransaction(command);
    ...
    this.auditLog.save({
      ...
      reason: mapReason(result.result === 'DENIED' ? result.reason : undefined),
      trace: {
        commandId: uuid(),
        correlationId: uuid(),
      },
